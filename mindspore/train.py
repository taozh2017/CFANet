...
class Trainer:
    ...
    def forward_fn(self, images, gts, egs):
        cam_edge, sal_out1, sal_out2, sal_out3 = self.model(images)
        loss_edge = self.loss_func(cam_edge, egs)
        ...
        return loss_total, loss_edge, loss_sal1, loss_sal2, loss_sal3

    def train_step(self, images, gts, egs):
        (loss, loss_edge, loss_sal1, loss_sal2, loss_sal3), grads = self.grad_fn(images, gts, egs)
        self.optimizer(grads)
        return loss, loss_edge, loss_sal1, loss_sal2, loss_sal3
        ...
        for epoch in range(1, epochs + 1):
            ...
            for step, data_pack in enumerate(self.train_loader.create_tuple_iterator(), start=1):
                ...
                for rate in self.size_rates:
                    ...
                    if rate != 1:
                        images = self.resize(images, size=(trainsize, trainsize), align_corners=True)
                        gts = self.resize(gts, size=(trainsize, trainsize), align_corners=True)
                        egs = self.resize(egs, size=(trainsize, trainsize), align_corners=True)
                    loss, loss_edge, loss_sal1, loss_sal2, loss_sal3 = self.train_step(images, gts, egs)
                ...
                if step % 10 == 0 or step == self.total_step:
                    print(
                        ...
                        format(datetime.now(), epoch, epochs, step, self.total_step, loss_edge.asnumpy(),
                               loss_sal1.asnumpy(), loss_sal2.asnumpy(), loss_sal3.asnumpy(), loss.asnumpy()))

                    logging.info(
                        '#TRAIN#:Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Loss_edge: {:.4f} Loss_sal1: {:0.4f} Loss_sal2: {:0.4f} Loss_sal3: {:0.4f} Loss_total: {:0.4f}'.
                        format(epoch, opt.epoch, step, self.total_step, loss_edge.asnumpy(), loss_sal1.asnumpy(),
                               loss_sal2.asnumpy(), loss_sal3.asnumpy(), loss.asnumpy()))
